...
- 中科星图 (Geovis Earth) - Add minute-level rain nowcast (Rain-Pulse, China only): the precipitation nowcasting card and the rain reminder are now available when Geovis is selected as the minutely source.
- 中科星图 (Geovis Earth) - Known limitation: the daily and hourly products are generated by different upstream models, so the sum of a day's hourly precipitation may not equal the daily total (in both directions). Both values are reported as returned by the API.
- QWeather - Alert headlines are shortened to their core "{type}{level}预警" form (e.g. "平南县气象台更新雷电黄色预警" -> "雷电黄色预警"); the change also benefits the China, Caiyun and Geovis sources.
