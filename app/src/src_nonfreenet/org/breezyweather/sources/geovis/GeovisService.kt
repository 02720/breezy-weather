...
class GeovisService @Inject constructor(
    ...
    override fun requestWeather(
        ...
        val minutely = if (SourceFeature.MINUTELY in requestedFeatures) {
            // The nowcast product is China-only, so it is only requested for Chinese locations.
            api.getMinutely(position, token)
                .map { it.apply { throwOnErrorCode(status) } }
                .onErrorResumeNext {
                    failedFeatures[SourceFeature.MINUTELY] = it
                    Observable.just(GeovisMinutelyResult())
                }
        } else {
            Observable.just(GeovisMinutelyResult())
        }
        ...
        return Observable.zip(
            ...
            airQuality,
            alert
        ) { currentResult, dailyResult, hourlyResult, minutelyResult, airQualityResult, alertResult ->
            WeatherWrapper(
    ...
    /**
     * Maps the daily product. The `pre_day`/`pre_night` amounts are the official day/night
     * precipitation totals of that product.
     *
     * Known limitation (checked 2026-08-07, official data): the daily product and the hourly
     * product (`getHourlyList`) are generated by different upstream models and are NOT
     * mass-consistent — the sum of a day's hourly `pre` values frequently does not equal the
     * daily `pre_day + pre_night` (in both directions, e.g. hourly all zero while the daily
     * total is tens of millimetres). Both values are faithfully reported as returned by the
     * API; the daily total is deliberately not recomputed from the hourly data, because the
     * hourly window only covers the hours from now on for the current day.
     */
    ...
    private fun getMinutelyList(
        result: GeovisMinutelyResult,
        location: Location,
    ): List<Minutely>? {
        val data = result.result ?: return null
        val start = data.start ?: return null
        if (start.isEmpty() || data.series.isNullOrEmpty()) return null
        val startDate = start.toGeovisDate(MINUTELY_DATE_PATTERN, location.timeZone) ?: return null
        // The series holds the precipitation intensity in mm/h for each minute from `start`
        // (inclusive) onwards; a point of the no-rain case is the empty series.
        return data.series.mapIndexed { minute, intensity ->
            Minutely(
                date = Date(startDate.time + minute * MINUTELY_STEP_MILLIS),
                minuteInterval = 1,
    ...
    companion object {
        ...
        private const val MINUTELY_DATE_PATTERN = "yyyyMMddHHmm"
        private const val ALERT_DATE_PATTERN = "yyyy-MM-dd HH:mm:ss"
        private const val MINUTELY_STEP_MILLIS = 60_000L
    }
}
